...
cell_archs = sorted(
    name
    for name in cell_models.__dict__
    if name.islower()
    and not name.startswith("__")
...
def get_args_parser():
    ...
    parser.add_argument("--sample_single_cells", action="store_true")
    parser.add_argument(
        "--center_momentum",
        default=0.9,
        type=float,
    ...
    parser.add_argument(
        ...
        default="folder",
        type=str,
        choices=["folder", "png_file_list", "tif_file_list"],
    ...
    parser.add_argument(
        "--root_dir_path",
        default="/home/ubuntu/data/CellNet_data/Hirano3D_v2.0/data/",
        type=str,
        help="root_dir_path for cells_dataloader.",
    )
    parser.add_argument(
        "--RGBmode", default=False, type=utils.bool_flag, help="""enforce 3-channels"""
    )
    parser.add_argument(
        "--scale-factor",
        default=1.0,
        type=float,
        help="""Set factor by which to resize source images.""",
    )
...
#        collate_fn = custom_collate,
    ...
    # otherwise, we check if the architecture is in torchvision models
        ...
        student = partial(cell_models.__dict__[args.arch], c_in=args.num_channels)(
            False
        )
        teacher = partial(cell_models.__dict__[args.arch], c_in=args.num_channels)(
            False
        )
